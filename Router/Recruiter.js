...
router.put("/savecandidate/:userId", jwtMiddleWare, async (req, res) => {
  try {
    ...
    if (user.usertype !== "recruiter") {
      return res
        .status(403)
        .json({ message: "Only recruiters can save candidates" });
    }
...
router.put("/updatejob/:id", jwtMiddleWare, async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Job Update Succssfully", UpdatedData: response });
  } catch (e) {
    console.log("error", e);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
    ...
    try {
      ...
      if (req.files.profilphoto?.[0]) {
        const result = await uploadToCloudinary(
          req.files.profilphoto[0].buffer,
          "recruiters/profile",
          "image"
        );
      ...
      if (
        ...
        const uploadPromises = req.files.recruterCompanyDoc.map((file) =>
          uploadToCloudinary(file.buffer, "recruiters/companyDocs", "raw")
        );
      ...
      Object.keys(userUpdatedData).forEach((key) => {
        if (userUpdatedData[key] === "") {
          delete userUpdatedData[key];
        }
      });
      ...
      return res
        .status(200)
        .json({ msg: "User updated successfully", user: response });
    } catch (e) {
...
router.post("/createplan", async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ msg: "Plan created successfully", plan: savedPlan });
  } catch (e) {
    console.log("error", e);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
...
router.put("/updateapplyjobstatus/:jobId", jwtMiddleWare, async (req, res) => {
  try {
    ...
    const candidate = job.candidatesApplied.find((candidate) =>
      candidate.userId.equals(candidateId)
    );
    if (!candidate) {
...
router.get("/getActiveSubscription", jwtMiddleWare, async (req, res) => {
  try {
    ...
    return res.status(200).json({
      msg: "Active Subscription Fetched Successfully!",
...
router.get("/getPaymentHistory", jwtMiddleWare, async (req, res) => {
  try {
    const userId = req.jwtPayload.id;
    const payments = await Payments.find({ userId: userId }).populate(
      "subscriptionId"
    );
    if (!payments) {
...
router.get("/dbpointUser", jwtMiddleWare, async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "DB Users Fetched Successfully!", users: dbUsers });
  } catch (e) {
    console.log("error", e);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
